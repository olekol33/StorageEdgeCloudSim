...
public class SimLogger {
	...
	static public void measureDuration(long startTime, String measuredName){
		if (!SimSettings.getInstance().isMeasureSimulationRuntime())
			return;
...
//				ioTaskID[key-1] = value.getIoTaskID(); //not really needed, just for debug below
			...
			if (value.getStatus() == SimLogger.TASK_STATUS.COMPLETED) {
				...
				parityTask[value.getTaskType()] += value.getIsParity();
		...
		parityTask[numOfAppTypes] = IntStream.of(parityTask).sum();
...
//							System.out.println("ioTaskID = " + key + ", data = " + list.get(0) + ", parity = " + readDelay + ", read parity");
			...
			for (int i = 0; i < numOfAppTypes + 1; i++) {
				...
				if(MATLAB) {
					...
					appendToFile(genericBWs[i], "#completedTask;parityTask;failedTask;uncompletedTask;failedTaskDuetoBw;serviceTime;processingTime;networkDelay;" +
					...
					appendToFile(genericBWs[i], genericResult1);
					appendToFile(genericBWs[i], "#completedTaskOnEdge;parityTask;failedTaskOnEdge;uncompletedTaskOnEdge;0;" +
							"serviceTimeOnEdge;processingTimeOnEdge;0;vmLoadOnEdgefailedTaskDueToVmCapacityOnEdge");
					appendToFile(genericBWs[i], genericResult2);
					appendToFile(genericBWs[i], "#completedTaskOnCloud;parityTask;failedTaskOnCloud;uncompletedTaskOnCloud;0;" +
							"serviceTimeOnCloud;processingTimeOnCloud;0;vmLoadOnClould;failedTaskDueToVmCapacityOnCloud");
					appendToFile(genericBWs[i], genericResult3);
					appendToFile(genericBWs[i], "#completedTaskOnMobile;parityTask;failedTaskOnMobile;uncompletedTaskOnMobile;0;serviceTimeOnMobile;" +
