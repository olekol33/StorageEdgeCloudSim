...
def isRunValid(folderName,threshold):
    ...
    failedRatio = lostTasksDF.shape[0]/tasksDF.shape[0]
...
def getNumOfClients(folderName):
    files = getLogsByNameFromFilepath(folderName, '_OBJECTS')
    return len(files)
...
def plotOrbitNSF():
    ...
    handleNSFObjectsRead(lambdas,failedLambdas,folderNames,folderPath,clients/2)
